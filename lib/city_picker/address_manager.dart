...
abstract class AddressManager {
  ...
  static Future<List<AddressProvince>> loadAddressData(
    ...
    var address = await rootBundle.loadString('assets/address.json');
    ...
    if (json != null && data is List) {
      data.forEach((v) {
        var province = AddressProvince.fromJson(v,
            cityMap: _cityMap, districtMap: _districtMap);
  ...
  static Future<AddressProvince> getProvince(
  ...
  static Future<AddressCity> getCity(
      BuildContext context, String cityId) async {
    if (_cityMap.isEmpty) {
      var provinces = await loadAddressData(context);
      if (provinces.isNotEmpty) {
  ...
  static Future<AddressDistrict> getDistrict(
